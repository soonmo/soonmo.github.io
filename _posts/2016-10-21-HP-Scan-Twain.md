...
HP에는 많은 복합기 제품이 있는데 크게 **"프로 (Pro)"** 모델과 **"엔터프라이즈 (Enterprise)"** 모델 2가지로 구분을 할 수 있다. 모델명을 보면 프로나 엔터프라이즈가 붙어 있다. 쉽게 말해서 엔터프라이즈 모델은 보안 기능과 같은 대기업에서 요구하는 기능들이 다수 포함되어 있고 가격이 프로 모델 보다 각격이 높게 형성되어 있다. 두 가지 제품군에 대한 구분은 추후에 따로 게시하기로 하겠다.
...
복사기 업체의 디지털 복합기도 크게 다르지 않다. 다만 **메일 박스**라고 해서 복합기에서 스캔을 하고 복합기가 서버의 역할을 하면서 PC에서 소프트웨어를 통해서 가져가는 방식이 존재하지만 이 또한 보안 이슈의 소지가 있기 때문에 대기업에서는 허용을 하지 않는 경우가 많다. 
...
![스캔 드라이버 다운로드](../images/HP-Scan-Twain_02.png)
